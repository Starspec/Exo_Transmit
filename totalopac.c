...
void TotalOpac() {
  ...
  if(chemSelection[0] == 1){          //If CH4 is selected
    ...
    /* Declare abundance */
    opacCH4.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
	/* Fill with chemistry abundance */
	opacCH4.abundance[j] = chem.CH4[j];  
    }
  ...
  if(chemSelection[1] == 1){
    ...
    opacCO2.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
	opacCO2.abundance[j] = chem.CO2[j];
    }
  ...
  if(chemSelection[2] == 1){
    ...
    opacCO.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[3] == 1){
    ...
    opacH2O.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[4] == 1){
    ...
    opacNH3.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[5] == 1){
    ...
    opacO2.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[6] == 1){
    ...
    opacO3.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[7] == 1){
    ...
    opacC2H2.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[8] == 1){
    ...
    opacC2H4.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[9] == 1){
    ...
    opacC2H6.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[10] == 1){
    ...
    opacH2CO.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[11] == 1){
    ...
    opacH2S.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[12] == 1){
    ...
    opacHCl.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[13] == 1){
    ...
    opacHCN.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[14] == 1){
    ...
    opacHF.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[15] == 1){
    ...
    opacMgH.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[16] == 1){
    ...
    opacN2.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[17] == 1){
    ...
    opacNO.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[18] == 1){
    ...
    opacNO2.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[19] == 1){
    ...
    opacOCS.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[20] == 1){
    ...
    opacOH.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[21] == 1){
    ...
    opacPH3.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[22] == 1){
    ...
    opacSH.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[23] == 1){
    ...
    opacSiH.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[24] == 1){
    ...
    opacSiO.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[25] == 1){
    ...
    opacSO2.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[26] == 1){
    ...
    opacTiO.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[27] == 1){
    ...
    opacVO.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[28] == 1){
    ...
    opacNa.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[29] == 1){
    ...
    opacK.abundance = dvector(0, NPRESSURE-1);
    for(j=0; j<NPRESSURE; j++){
  ...
  if(chemSelection[31] == 1){	      //If CIA is turned on ...
    ...
    opacCIA.abundance = dvector(0, NPRESSURE-1);
    ...
    for(i=0; i<NPRESSURE; i++){
      fscanf(f1, "%le", &opacCIA.P[i]);
    }
    for (k=0; k<NTEMP; k++){
      fscanf(f1, "%le", &opacCIA.T[k]);
      for (i=0; i<NLAMBDA; i++){
        fscanf(f1, "%le %le %le %le %le %le %le %le %le %le %le %le %le %le %le",
                &atmos.lambda[i], &opac_CIA_H2H2[i][k],
                &opac_CIA_H2He[i][k], &opac_CIA_H2H[i][k],
                &opac_CIA_H2CH4[i][k], &opac_CIA_CH4Ar[i][k],
                &opac_CIA_CH4CH4[i][k], &opac_CIA_CO2CO2[i][k],
                &opac_CIA_HeH[i][k], &opac_CIA_N2CH4[i][k],
                &opac_CIA_N2H2[i][k], &opac_CIA_N2N2[i][k],
                &opac_CIA_O2CO2[i][k], &opac_CIA_O2N2[i][k],
                &opac_CIA_O2O2[i][k]);
      }
    }
    ...
    for (i=0; i<NLAMBDA; i++){
      for (j=0; j<NPRESSURE; j++){
          for (k=0; k<NTEMP; k++) {
	  opacCIA.kappa[i][j][k] += opac_CIA_H2H2[i][j] * 
	    chem.H2[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]) * 
	    chem.H2[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]);
	  opacCIA.kappa[i][j][k] += opac_CIA_H2He[i][j] * 
	    chem.H2[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]) * 
	    chem.He[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]);
	  opacCIA.kappa[i][j][k] += opac_CIA_H2H[i][j] * 
	    chem.H2[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]) * 
	    chem.H[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]);
	  opacCIA.kappa[i][j][k] += opac_CIA_H2CH4[i][j] * 
	    chem.H2[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]) * 
	    chem.CH4[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]);
	  /* opacCIA.kappa[i][j][k] += opac_CIA_CH4Ar[i][j] *  */
	  /*   chem.CH4[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]) *  */
	  /*   chem.Ar[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]); */
	  opacCIA.kappa[i][j][k] += opac_CIA_CH4CH4[i][j] * 
	    chem.CH4[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]) * 
	    chem.CH4[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]);
	  opacCIA.kappa[i][j][k] += opac_CIA_CO2CO2[i][j] * 
	    chem.CO2[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]) * 
	    chem.CO2[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]);
	  opacCIA.kappa[i][j][k] += opac_CIA_HeH[i][j] * 
	    chem.He[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]) * 
	    chem.H[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]);
	  opacCIA.kappa[i][j][k] += opac_CIA_N2CH4[i][j] * 
	    chem.N2[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]) * 
	    chem.CH4[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]);
	  opacCIA.kappa[i][j][k] += opac_CIA_N2H2[i][j] * 
	    chem.N2[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]) * 
	    chem.H2[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]);
	  opacCIA.kappa[i][j][k] += opac_CIA_N2N2[i][j] * 
	    chem.N2[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]) * 
	    chem.N2[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]);
	  opacCIA.kappa[i][j][k] += opac_CIA_O2CO2[i][j] * 
	    chem.O2[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]) * 
	    chem.CO2[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]);
	  opacCIA.kappa[i][j][k] += opac_CIA_O2N2[i][j] * 
	    chem.O2[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]) * 
	    chem.N2[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]);
	  opacCIA.kappa[i][j][k] += opac_CIA_O2O2[i][j] * 
	    chem.O2[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]) * 
	    chem.O2[j] * opac.P[j] / (KBOLTZMANN * opac.T[j]);
      }
    }
  ...
  if(chemSelection[30] == 1){		//If Scattering is activated.. 
    ...
    opacscat.abundance = dvector(0, NPRESSURE-1);
